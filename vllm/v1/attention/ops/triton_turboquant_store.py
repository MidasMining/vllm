...
def _tq_mla_fused_store_fp8_kernel(
    ...
    bit-identical bytes will dequantize to different values. e5m2 has
    slightly worse mantissa precision for kv_c keys but is the only fp8
    Triton type that loads natively on SM<89.
    """
    ...
    if FP8_E5M2:
        kv_c_fp8 = kv_c_clamped.to(tl.float8e5)
    else:
    ...
    if K_PE_FP8:
        ...
        if FP8_E5M2:
            k_pe_fp8 = kpe_clamped.to(tl.float8e5)
        else:
...
def _tq_mla_fused_store_mse_kernel(
    ...
    BLOCK_L: tl.constexpr,
    BLOCK_R: tl.constexpr,
    FP8_E5M2: tl.constexpr = 0,  # 1 = e5m2 (Ampere/Volta), 0 = e4nv (Hopper+).
):
    ...
    else:  # MSE_BITS == 2 — 4 indices per byte at shifts 0/2/4/6
        idx_quads = tl.reshape(idx, [BLOCK_L // 4, 4])
        shifts_2 = tl.arange(0, 4) * 2
        packed = tl.sum((idx_quads & 0x3) << shifts_2[None, :], axis=1).to(tl.uint8)
        mse_offs = tl.arange(0, BLOCK_L // 4)
        mse_mask = mse_offs < MSE_BYTES
        tl.store(kv_cache_ptr + cache_offset + mse_offs, packed, mask=mse_mask)
...
def _mla_fused_store_fp8(
    ...
    fp8_e5m2 picks the fp8 type for both kv_c and k_pe writes:
        False (default) → tl.float8e4nv (Hopper-native)
        True            → tl.float8e5   (Ampere-native)
    Read kernel must use the matching type.
    """
    N = kv_c.shape[0]
    L = kv_lora_rank
    R = qk_rope_head_dim
...
def _mla_fused_store_mse(
    ...
    fp8_e5m2 only matters when k_pe_fp8=True; it picks tl.float8e5
    (Ampere-native) instead of tl.float8e4nv (Hopper-only) for the k_pe
    write. Read kernel must use the matching type.
    """
    N = kv_c.shape[0]
    L = kv_lora_rank
    R = qk_rope_head_dim
