...
A ~180 GB mixture-of-experts model serving 256k-token contexts at 90–165
tokens/s on four ex-mining GPUs over PCIe Gen2 with no NVLink. Measured
...
| 200k-token prefill | ~43 s (~4,650 tok/s) |
...
The context limit is set to 262,144 tokens; the model's native limit is 1M.
...
| **TurboQuant fp8 KV + private drafter pool (current)** | **1,928,026** | **7.35** |
...
- `bench/`: raw decode, parallel and practical-suite JSON for the numbers above
